...
import torch
from DRLTRFeatureGenerator import DataGetter
from  DRLTREnv import SingleAssetTradingEnvironment
from DRLTRAgentMemory import ReplayMemory, Transition
from DRLTRDQNAgent import DQNAgent, DuellingDQN
...
for episode in range(1, 1 + N_EPISODES):
  ...
  def visualize_portfolio_wealth(episode_scores_train, episode_scores_test):
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, N_EPISODES + 1), episode_scores_train, label="Training")
    plt.plot(range(1, N_EPISODES + 1), episode_scores_test, label="Testing")
    plt.xlabel("Episode")
    plt.ylabel("Portfolio Wealth ($)")
    plt.title("Portfolio Wealth during Training and Testing")
    plt.legend()
    plt.grid(True)
    plt.show()

  visualize_portfolio_wealth(episode_scores_train, episode_scores_test)
